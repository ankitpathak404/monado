...
/*!
 * @file
 * @brief Standalone Vulkan bundle initialization.
 *
...
static VkResult
fill_in_results(struct vk_bundle *vk, const struct vk_bundle_init_arguments *vk_args, struct vk_bundle_init_results *vk_res)
{
...
static VkResult
create_instance(struct vk_bundle *vk, const struct vk_bundle_init_arguments *vk_args)
{
...
static VkResult
create_device(struct vk_bundle *vk, const struct vk_bundle_init_arguments *vk_args)
{
	VkResult ret;
	...
	for (size_t i = 0; i < ARRAY_SIZE(prios); i++) {
		...
		// All ok!
		if (ret == VK_SUCCESS) {
			VK_DEBUG(vk, "Created device and %s queue with %s.", only_compute_queue ? "COMPUTE" : "GRAPHICS",
			        prio_strs[i]);
			break;
		}
	...
	vk_print_opened_device_info(vk, U_LOGGING_DEBUG);

	// Print selected queue(s) information.
	vk_print_queues_info(vk, U_LOGGING_DEBUG);

	// Print features enabled.
	vk_print_features_info(vk, U_LOGGING_DEBUG);

	// Now that we are done debug some used external handles.
	vk_print_external_handles_info(vk, U_LOGGING_DEBUG);

	return VK_SUCCESS;
}

bool
vk_bundle_init_from_scratch(struct vk_bundle *vk,
                            const struct vk_bundle_init_arguments *vk_args,
                            struct vk_bundle_init_results *vk_res)
{
	VkResult ret;
