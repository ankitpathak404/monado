...
simulated_hmd_create(enum simulated_movement movement, const struct xrt_pose *center)
{
	...
	// Blend modes - ordered most preferred first.
	// ALPHA_BLEND enables passthrough / transparent background in StereoKit.
	size_t bm_idx = 0;
	hmd->base.hmd->blend_modes[bm_idx++] = XRT_BLEND_MODE_ALPHA_BLEND;
	hmd->base.hmd->blend_modes[bm_idx++] = XRT_BLEND_MODE_OPAQUE;
	hmd->base.hmd->blend_mode_count = bm_idx;
	U_LOG_I("SIMULATED HMD: Initialized with %zu blend modes (ALPHA_BLEND supported)", bm_idx);
