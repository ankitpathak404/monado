...
#include "util/u_hardcoded_hand.h"
...
#include "../../tracking/mediapipe/mp_interface.h"

static void
hardcoded_sync_process(struct t_hand_tracking_sync *ht_sync,
...
{
	// Dummy process callback: completely ignores incoming camera frames
	// and fills output with hardcoded joint data instead.
	u_hand_joint_set_fill_hardcoded(out_left_hand, false);
	u_hand_joint_set_fill_hardcoded(out_right_hand, true);
	*out_timestamp_ns = os_monotonic_get_ns();
}

static void
hardcoded_sync_destroy(struct t_hand_tracking_sync *ht_sync)
{
	free(ht_sync);
}
                            ...
                            int64_t *out_timestamp_ns)
{
	...
	/*
	 * Bypass logic for MERCURY_HARDCODE. We intercept the tracking request here
	 * and return hardcoded data immediately. This bypasses the async wrapper
	 * which usually hangs if no camera frames are being pushed.
	 */
	const char *hardcode_env = getenv("MERCURY_HARDCODE");
	if (hardcode_env && (strcmp(hardcode_env, "true") == 0 || strcmp(hardcode_env, "1") == 0)) {
		u_hand_joint_set_fill_hardcoded(out_value, name == XRT_INPUT_HT_UNOBSTRUCTED_RIGHT);
		*out_timestamp_ns = os_monotonic_get_ns();
		return XRT_SUCCESS;
	}
                 ...
                 struct xrt_device **out_device)
{
	...
	const char *hardcode_env = getenv("MERCURY_HARDCODE");
	if (hardcode_env && (strcmp(hardcode_env, "true") == 0 || strcmp(hardcode_env, "1") == 0)) {
		U_LOG_I("ht_driver: using HARDCODED hand tracking bypass");
		sync = U_TYPED_CALLOC(struct t_hand_tracking_sync);
		sync->process = hardcoded_sync_process;
		sync->destroy = hardcoded_sync_destroy;
	} else {

	/* Choose backend via environment variable:
	 *   XRT_HAND_TRACKING_BACKEND=mediapipe  → use MediaPipe (mp.py)
	 *   (anything else, or unset)            → use Mercury (ONNX model)
	 */
	const char *backend = getenv("XRT_HAND_TRACKING_BACKEND");
	if (backend && strcmp(backend, "mediapipe") == 0) {
		U_LOG_I("ht_driver: using MediaPipe hand tracking backend");
		const char *script_path = getenv("XRT_MP_SCRIPT_PATH");
		sync = t_hand_tracking_sync_mediapipe_create(script_path);
		if (!sync) {
			U_LOG_E("ht_driver: failed to create MediaPipe backend");
			return -1;
		}
	} else {
		char path[1024] = {0};
		...
		U_LOG_I("ht_driver: using Mercury hand tracking backend");
