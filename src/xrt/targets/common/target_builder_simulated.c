                           ...
                           struct u_builder_roles_helper *ubrh)
{
...
#ifdef XRT_BUILD_DRIVER_HANDTRACKING
	{
		...
		bool hardcode_active = hardcode_env && (strcmp(hardcode_env, "true") == 0 || strcmp(hardcode_env, "1") == 0);

		if (hardcode_active) {
			struct t_stereo_camera_calibration *dummy_calib = NULL;
			t_stereo_camera_calibration_alloc(&dummy_calib, T_DISTORTION_OPENCV_RADTAN_8);
			
			struct xrt_slam_sinks *sinks = NULL;
			struct t_hand_tracking_create_info create_info = {0};
			struct xrt_device *ht_dev = NULL;
			
			int ht_ret = ht_device_create(xfctx, dummy_calib, create_info, &sinks, &ht_dev);
			
			if (ht_ret == 0 && ht_dev != NULL) {
				xsysd->xdevs[xsysd->xdev_count++] = ht_dev;
				ubrh->hand_tracking.unobstructed.left = ht_dev;
				ubrh->hand_tracking.unobstructed.right = ht_dev;
				U_LOG_I("Simulated builder: Created hardcoded Hand Tracker device.");
			}
		}
	}
#endif
